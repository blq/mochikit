 ...
 * @return {!Iterator}
 */
MochiKit.Iter.count = function (n, step/* = 1 */) {
    if (!n) {
        n = 0;
    }
	if (arguments.length < 2)
 ...
 * @return {!Iterator}
 */
MochiKit.Iter.izip = function (p, var_args/*, ...*/) {
    var m = MochiKit.Base;
    var self = MochiKit.Iter;
    var next = self.next;
 ...
 * @return {!Iterator}
 */
MochiKit.Iter.imap = function (fun, p, var_args/*, ...*/) {
    var m = MochiKit.Base;
    var self = MochiKit.Iter;
 ...
 * @return {!Iterator}
 */
MochiKit.Iter.chain = function (p, var_args/*, ...*/) {
    // dumb fast path
    var self = MochiKit.Iter;
    var m = MochiKit.Base;
 ...
 * @return {!Array}
 */
MochiKit.Iter.iextend = function (lst, iterable, /* optional */skip) { // todo: add support for a start pos? similar to Base.extend, resulting in a kindof islice/iextend combo.
	skip = skip || 0;
    var m = MochiKit.Base;
    var self = MochiKit.Iter;
    if (m.isArrayLike(iterable) && !self.isIterable(iterable)) {
 ...
 * @return {!Iterator}
 */
MochiKit.Iter.groupby = function(iterable, /* optional */ keyfunc) {
    var m = MochiKit.Base;
    var self = MochiKit.Iter;
    if (arguments.length < 2) {
 ...
 * @return {!Array}
 */
MochiKit.Iter.groupby_as_array = function (iterable, /* optional */ keyfunc) {
    var m = MochiKit.Base;
    var self = MochiKit.Iter;
    if (arguments.length < 2) {
        keyfunc = m.operator.identity;
 ...
 * @return {boolean}
 */
MochiKit.Iter.hasIterateNext = function (iterable) {
 ...
 * @return {!Iterator}
 */
MochiKit.Iter.iterateNextIter = function (iterable) {
    return {
